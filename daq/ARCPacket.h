...
#define PFX_CHIP_LAST_CELL_READ_ARC                                            \
  0x1800 // Chip index (2 bit) + last cell read (9 bits)
...
#define PFX_START_OF_DFRAME                                                    \
  0x0800 // Start of Data Frame + 3 bit Version + 1 bit source type + 5 bit
         // source
#define PFX_START_OF_MFRAME                                                    \
  0x0600 // Start of Monitoring Frame + 3 bit Version + 1 bit source type + 5
         // bit source
#define PFX_START_OF_CFRAME                                                    \
  0x0400 // Start of Configuration Frame + 3 bit Version + 1 bit source type + 5
         // bit source
...
#define PFX_START_OF_EVENT_ARC                                                 \
  0x0300 // Start Of Event + 2 bit event type + 1 bit source type + 5 bit source
         // ID

// Prefix Codes for 6-bit data content
#define PFX_6_BIT_CONTENT_MASK 0xFFC0 // Mask to select 10 MSB's of prefix
#define PFX_END_OF_EVENT_ARC                                                   \
  0x02C0 // End Of Event + 1 bit source type + 5 bit source ID
...
#define PFX_START_OF_EVENT_MINOS                                               \
  0x00F0 // Start of Event + 1 bit free + Event Trigger Type
#define PFX_END_OF_EVENT_MINOS 0x00E0 // End of Event + 4 MSB of size
#define PFX_EXTD_CARD_CHIP_LAST_CELL_READ                                      \
  0x00D0 // Last Cell Read + 4 bit Card/Chip index followed by 16-bit last cell
         // value
...
#define PFX_EXTD_CARD_CHIP_CHAN_H_MD                                           \
  0x0012 // Header for pedestal histogram of one channel - mean and std
         // deviation
#define PFX_EXTD_CARD_CHIP_CHAN_HIT_IX 0x0011 // Header for data of one channel
#define PFX_EXTD_CARD_CHIP_CHAN_HISTO                                          \
  0x0010                        // Header for pedestal histogram of one channel
...
void DataPacket_Print(uint16_t *fr, const uint16_t &size, FILE *fp = stdout);
uint32_t GetUInt32FromBuffer(uint16_t *&fr, int &sz_rd, bool BE = false);
