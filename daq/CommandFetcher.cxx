...
void CommandFetcher::handleCommand(const std::string &line) {
  ...
  if (cmd == "exit" || cmd == "quit") {
    ...
    auto result = std::from_chars(args[0].data(),
                                  args[0].data() + args[0].size(), sleepTime);
...
bool CommandFetcher::interrupted() { return g_interrupted.load(); }

bool CommandFetcher::shutdownRequested() { return g_shutdown.load(); }
