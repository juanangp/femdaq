...
typedef struct {
  unsigned short
      size; // size in bytes including the size of the size field itself
  ...
  unsigned short scnt;   // sample count in first pulse over thr or total sample
                         // count in packet
  unsigned short samp[MAX_ETH_PACKET_DATA_SIZE /
                      (sizeof(unsigned short))]; // data storage area
} DataPacket;
...
typedef struct {
  ...
  channel_stat
      stat[ASIC_MAX_CHAN_INDEX + 1]; // summary of statistics for current ASIC
