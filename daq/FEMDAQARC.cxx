        ...
        // protect socket operations
          {
            ...
            if(length<= 6)continue; //empty frame?
              ...
              if (runConfig.verboseLevel >= RunConfig::Verbosity::Debug)ARCPacket::DataPacket_Print(&buf_rcv[1], size-1);
                ...
                if(ARCPacket::isDataFrame(&buf_rcv[1])) {
                  ...
                  const size_t bufferSize = FEM.buffer.size();
                  FEM.mutex_mem.unlock();
                  if (runConfig.verboseLevel >= RunConfig::Verbosity::Info)ARCPacket::DataPacket_Print(&buf_rcv[1], size-1);
                } else {
  ...
  while(!(emptyBuffer && stopDAQ)) {
    ...
    newEvent = true;
      ...
      for (auto &FEM : FEMArray){
       ...
       stopDAQ = FEMDAQ::abrt || !(runConfig.nEvents == 0 || ev_count < runConfig.nEvents);
      ...
      if(newEvent){//Save Event if closed
        ...
        if (!runConfig.readOnly){
          ...
          if(ev_count%100 == 0)std::cout<<"Events "<<ev_count<<std::endl;
