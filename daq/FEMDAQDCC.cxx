...
void FEMDAQDCC::startDAQ( const std::string &flags ){
  ...
  char cmd[200];
    ...
    while (!FEMDAQ::abrt && (runConfig.nEvents == 0 || FEMDAQ::storedEvents.load() < runConfig.nEvents) ){
        ...
        if(internal)SendCommand("isobus 0x1C",FEM);
        sEvent.Clear();
...
DCCPacket::packetReply FEMDAQDCC::SendCommand(const char* cmd, FEMProxy &FEM, DCCPacket::packetType pckType, size_t nPackets, DCCPacket::packetDataType dataType) {
    ...
    while (!done) {
        ...
        if (duration.count() >= 10 || length < 0) {
            std::cout << "No reply after " << duration.count() << " seconds, missing packets are expected" << std::endl;
...
void FEMDAQDCC::waitForTrigger() {  // Wait till trigger is acquired
    ...
    do {
        reply = SendCommand("wait 1000000", FEM);                   // Wait for the event to be acquired
