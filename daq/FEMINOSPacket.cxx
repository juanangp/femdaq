...
void DataPacket_Print(uint16_t *fr, const uint16_t &size, FILE *fp) {
  ...
  do {
    if ((*fr & PFX_14_BIT_CONTENT_MASK) == PFX_CARD_CHIP_CHAN_HIT_IX) {
      ...
      r1 = GET_CHIP_IX(*fr);
      r2 = GET_CHAN_IX(*fr);
      fprintf(fp, "Card %02d Chip %01d Channel_Hit_Count %02d\n", r0, r1, r2);
      fr++;
      sz_rd++;
      ...
      for (int i = 0; i < 4; i++) {
        fprintf(fp, "Chip %01d Last_Cell_Read %03d (0x%03x)\n",
                GET_LST_READ_CELL_CHIP_IX(*fr), GET_LST_READ_CELL(*fr),
                GET_LST_READ_CELL(*fr));
        fr++;
        sz_rd++;
      }
      ...
      fr++;
      sz_rd++;
      fprintf(fp, "--- Start of Data Frame (V.%01d) FEM %02d --\n", r0, r1);
      ...
      fr++;
      sz_rd++;
      fprintf(fp, "--- Start of Moni Frame (V.%01d) FEM %02d --\n", r0, r1);
      fprintf(fp, "Filled with %d bytes\n", *fr);
      fr++;
      sz_rd++;
      ...
      fr++;
      sz_rd++;
      fprintf(fp, "--- Start of Config Frame (V.%01d) FEM %02d --\n", r0, r1);
...
void ParseEventFromWords(std::deque<uint16_t> &event, SignalEvent &sEvent,
  ...
  while (idx < buffSize) {
    ...
    if ((w & PFX_9_BIT_CONTENT_MASK) == PFX_START_OF_DFRAME) {
      ...
      // std::cout<<std::hex<<"0x"<<event[idx]<<std::dec<<" Card "<<cardID<<"
      // Chip "<<chipID<<" Channel "<<chID<<" PhysChann
