...
void GetParamsFromPulse(const std::vector<short> &pulse, double &amplitude,
                        double &area, int &maxPos) {
  ...
  if (max < baseLine + threshold)
    return;

  // cout<<baseLine<<" "<<baseLineSigma<<" "<<max<<" "<< baseLine + threshold
  // <<endl;
...
void AnalyzePulses(const std::string &filePattern,
  ...
  auto hitmap = new TH2F("HitMap", "HitMap", nChannels, 0, nChannels, nChannels,
                         0, nChannels);
  can3->cd();
  hitmap->Draw();
  ...
  for (size_t entryID = 0; entryID < entries; entryID++) {
    ...
    for (size_t s = 0; s < signalsID->size(); s++) {
      ...
      std::vector<short> pulse(pulses->begin() + offset,
                               pulses->begin() + offset + 512);
      std::string hName = "Hist_" + std::to_string(signalID);
      GetParamsFromPulse(pulse, amplitude, area, maxPos);

      if (area == 0 || amplitude == 0)
        continue;

      // std::cout<<"Card "<<card<<" channel "<<channel<<" max
      // "<<amplitude<<endl;
      if (!decodingMap.empty()) {
        ...
        if (it != decodingMap.end()) {
          int pos = it->second;
          if (interactive)
            cout << "SID " << signalID << " --> " << pos << " " << area << endl;
          ...
          if (interactive) {
            if (pos / nChannels == 0)
              hitX->Fill(posChann, maxPos, area);
            else
              hitY->Fill(posChann, maxPos, area);
          }
        }
      }
      ...
      if (interactive) {
        std::string hName = "Hist_" + std::to_string(signalID);
        auto histo = new TH1S(hName.c_str(), hName.c_str(), pulse.size(), 0.,
                              pulse.size());
    ...
    if (interactive) {
      ...
      std::unique_ptr<TTimer> timer(
          new TTimer("gSystem->ProcessEvents();", 100, kFALSE));
      do {
        std::cout << "Type 'n' for next event or 'q' to quit interactive mode "
                  << std::endl;
        timer->TurnOn();
        timer->Reset();
        scanf("%s", str);
