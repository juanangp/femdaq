...
public:
  EventViewer() : TGMainFrame(gClient->GetDefaultRoot(), 1200, 900) {
    ...
    TGVerticalFrame *vFrame =
        new TGVerticalFrame(hFrame, 150, 900, kFixedWidth);
    ...
    // Event Input Section
    vFrame->AddFrame(new TGLabel(vFrame, "Jump to Event:"),
                     new TGLayoutHints(kLHintsCenterX, 5, 5, 10, 0));
    fEntryInput = new TGTextEntry(vFrame);
    fEntryInput->Connect("ReturnPressed()", "EventViewer", this,
                         "JumpToEvent()");
    vFrame->AddFrame(fEntryInput,
                     new TGLayoutHints(kLHintsExpandX, 5, 5, 2, 5));

    fInfoLabel = new TGLabel(vFrame, "No data");
    vFrame->AddFrame(fInfoLabel,
                     new TGLayoutHints(kLHintsCenterX, 5, 5, 20, 5));
    ...
    hFrame->AddFrame(
        vFrame, new TGLayoutHints(kLHintsLeft | kLHintsExpandY, 5, 5, 5, 5));

    // --- Canvas Area (Right) ---
    fEcan = new TRootEmbeddedCanvas("Ecan", hFrame, 1000, 850);
    hFrame->AddFrame(
        fEcan, new TGLayoutHints(kLHintsExpandX | kLHintsExpandY, 5, 5, 5, 5));
  ...
  void Next() {
    if (chain && currentEntry < chain->GetEntries() - 1) {
      currentEntry++;
      DrawEvent();
    }
  }
  ...
  void SavePDF() {
    if (chain)
      fEcan->GetCanvas()->SaveAs(Form("Event_%ld.pdf", currentEntry));
  }
  ...
  void DrawEvent() {
    ...
    fInfoLabel->SetText(
        Form("Entry: %ld / %lld", currentEntry, chain->GetEntries() - 1));
    ...
    for (size_t s = 0; s < signalsID->size(); s++) {
      ...
      // Unique name and SetDirectory(0) to avoid "Replacing existing TH1"
      // warnings
      TH1S *h = new TH1S(Form("h_%zu_e%ld", s, currentEntry), "", 512, 0, 512);
      h->SetDirectory(0);

      for (int p = 0; p < 512; p++)
        h->SetBinContent(p + 1, pulses->at(s * 512 + p));
...
// Start macro
void DrawPulses(const std::string &fileName = "") { new EventViewer(); }
