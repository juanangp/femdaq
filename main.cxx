...
int main(int argc, char** argv) {
  ...
  app.add_option("-c,--config-file", configFile, "Configuration file.")->group("General");
  app.add_flag("--read-only", readOnly, ("Read-only mode"))->group("General");

  CLI11_PARSE(app, argc, argv);

  if(configFile.empty()){
    std::cout<<"Please provide a valid config file "<<configFile<<std::endl;
    exit(1);
  }
  ...
  if(readOnly)runConfig.readOnly = true;
